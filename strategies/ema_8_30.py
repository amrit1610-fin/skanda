...
def generate_signals(df: pd.DataFrame) -> tuple[pd.Series, pd.Series]:
    ...
    ema_dist = (ema8 - ema30).abs()
    
    # Slope calculation: Is the 30 EMA actually moving? (Comparing current to 3 bars ago)
    ema30_slope = ema30.diff(3) 
    min_slope_threshold = atr * 0.05 # Require slope to be at least 5% of ATR
    ...
    # 5. Signal Generation
    buy_signal_bar = uptrend & long_pullback & (df['volume'] > vol_sma) & (ema_dist > (atr * 0.3))
    sell_signal_bar = downtrend & short_pullback & (df['volume'] > vol_sma) & (ema_dist > (atr * 0.3))
